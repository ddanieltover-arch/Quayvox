...
async function insertShipment(payload) {
  ...
  const rows = await sql`
    insert into public.shipments (
      ...
      dim_l, dim_w, dim_h, cost, cost_currency, eta, progress, mode, priority,
      ...
      ${resolved.dim_w},
      ${resolved.dim_h},
      ${Number(resolved.cost ?? 0)},
      ${asString(resolved.cost_currency || "USD") || "USD"},
      ${resolved.eta},
      ${resolved.progress},
      ${resolved.mode},
      ${resolved.priority},
...
async function updateShipment(id, patch) {
  ...
  const rows = await sql`
    update public.shipments set
      ...
      dim_w = ${withGeo.dim_w},
      dim_h = ${withGeo.dim_h},
      cost = ${Number(withGeo.cost ?? 0)},
      cost_currency = ${asString(withGeo.cost_currency || "USD") || "USD"},
      eta = ${withGeo.eta},
...
var createSchema = import_zod5.z.object({
  ...
  cost: import_zod5.z.number().nonnegative().optional().default(0),
  cost_currency: import_zod5.z.string().trim().toUpperCase().regex(/^[A-Z]{3}$/).optional().default("USD"),
...
var patchSchema = import_zod5.z.object({
  ...
  cost: import_zod5.z.number().nonnegative().optional(),
  cost_currency: import_zod5.z.string().trim().toUpperCase().regex(/^[A-Z]{3}$/).optional(),
  eta: import_zod5.z.string().nullable().optional(),
